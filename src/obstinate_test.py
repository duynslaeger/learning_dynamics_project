...
for rich_coop in tqdm(range(obstinate_coop[0], Zr + 1 - obstinate_defector[0])):
    for poor_coop in range(obstinate_coop[1], Zp + 1 - obstinate_defector[1]):
        ...
        multi_hypergeom_curr_composition = multivariate_hypergeom(
            m=[rich_coop, rich_defectors, poor_coop, poor_defectors], n=6)
        aG_i = 0.0
                        ...
                        if (rc + pc + rd <= N):
                            for pd in range(N):  # pd represent the number of poor defectors in the group
                                if (rc + pc + rd + pd == N):
                                    ...
                                    aG_i += pmf
        a_g[rich_coop * (Zp + 1) + poor_coop] = aG_i
...
plt.streamplot(X, Y, U, V, density=0.8, color=speed, linewidth=1, cmap='viridis')
